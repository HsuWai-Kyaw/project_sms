...
if (!empty($search_query)) {
     $enrollment_query .= " WHERE student_id LIKE :search_query OR head_of_enrollment LIKE :search_query";
}

$enrollment_query .= " LIMIT :start_page, :record_per_page";

$s = $pdo->prepare($enrollment_query);
...
<div class="header w-auto shadow m-auto p-3">
     <nav class="navbar navbar-expand-lg bg-body-tertiary">
          <div class="container-fluid">
               <a class="navbar-brand" href="enrollment_index.php">enrollment List</a>
               ...
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                         <li class="nav-item">
                              <a class="nav-link active" aria-current="page" href="enrollment_index.php">Home</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link" href="add_enrollment.php">Add New +</a>
                         </li>
     ...
     <table class="table m-3 align-middle mb-0 bg-white">
          ...
          <tbody>
               ...
               <?php foreach ($results as $key => $value) { ?>
                    ...
                    <th><?= ++$key ?></th>
                    <td scope="row"><?= $value['enrollment_id'] ?></td>
                    <?php
                    ...
                    <td scope="row"><?= $value['enrollment_date'] ?></td>
                    <td scope="row"><?= $value['status']?></td>
                    <td scope="row"><a href="enrollment_detail.php?id=<?= $value['enrollment_id'] ?>" class="btn btn-sm btn-info">Detail</a>
                    </td>
                    <td scope="row"> <a href="edit_enrollment.php?id=<?= $value['enrollment_id'] ?>" class="btn btn-sm btn-warning">Manage</a>

                    </td>
                    <td scope="row"><a href="delete.php?id=<?= $value['enrollment_id']; ?>" class="btn btn-sm btn-danger" onclick="alert('are you sure?')">Delete</a>
...
<div class="pagination m-auto" style="width: fit-content;">
     ...
     $page_qry = "SELECT * FROM enrollment";
     if (!empty($search_query)) {
          $page_qry .= " WHERE enrollment LIKE :search_query OR location LIKE :search_query";
     }
