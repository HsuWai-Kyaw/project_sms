...
$subject_query = "SELECT s.*, c.course_name FROM subject s
LEFT JOIN courses c ON c.course_id = s.course_id";

if (!empty($search_query)) {
     $subject_query .= " WHERE subject_name LIKE :search_query OR course_id LIKE :search_query";
}

$subject_query .= " LIMIT :start_page, :record_per_page";

$s = $pdo->prepare($subject_query);
...
<div class="header w-auto shadow m-auto p-3">
     <nav class="navbar navbar-expand-lg bg-body-tertiary">
          <div class="container-fluid">
               <a class="navbar-brand" href="subject_index.php">subject List</a>
               ...
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                         <li class="nav-item">
                              <a class="nav-link active" aria-current="page" href="subject_index.php">Home</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link" href="add_subject.php">Add New +</a>
                         </li>
     ...
     <table class="table m-3 align-middle mb-0 bg-white">
          ...
          <tbody>
               ...
               <?php foreach ($results as $key => $value) { ?>
                    ...
                    <th><?= ++$key ?></th>
                    <td scope="row"><?= $value['subject_id'] ?></td>
                    <td scope="row"><?= $value['subject_name'] ?></td>
                    ...
                    <td scope="row"><a href="subject_detail.php?id=<?= $value['subject_id'] ?>" class="btn btn-sm btn-info">Detail</a>
                    </td>
                    <td scope="row"> <a href="edit_subject.php?id=<?= $value['subject_id'] ?>" class="btn btn-sm btn-warning">Edit</a>

                    </td>
                    <td scope="row"><a href="delete.php?id=<?= $value['subject_id']; ?>" class="btn btn-sm btn-danger" onclick="alert('are you sure?')">Delete</a>
...
<div class="pagination m-auto" style="width: fit-content;">
     ...
     $page_qry = "SELECT * FROM subject";
     if (!empty($search_query)) {
          $page_qry .= " WHERE subject LIKE :search_query OR location LIKE :search_query";
     }
