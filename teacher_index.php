...
if (!empty($search_query)) {
     $teacher_query .= " WHERE teacher_id LIKE :search_query OR email LIKE :search_query";
}

$teacher_query .= " LIMIT :start_page, :record_per_page";

$s = $pdo->prepare($teacher_query);
...
<div class="header w-auto shadow m-auto p-3">
     <nav class="navbar navbar-expand-lg bg-body-tertiary">
          <div class="container-fluid">
               <a class="navbar-brand" href="teacher_index.php">teacher List</a>
               ...
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                         <li class="nav-item">
                              <a class="nav-link active" aria-current="page" href="teacher_index.php">Home</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link" href="add_teacher.php">Add New +</a>
                         </li>
     ...
     <table class="table m-3 align-middle mb-0 bg-white">
          ...
          <tbody>
               ...
               <?php foreach ($results as $key => $value) { ?>
                    ...
                    <th><?= ++$key ?></th>
                    <td scope="row"><?= $value['teacher_id'] ?></td>
                ...
                ?>
                    ...
                    <td scope="row"><a href="teacher_detail.php?id=<?= $value['teacher_id'] ?>" class="btn btn-sm btn-info">Detail</a>
                    </td>
                    <td scope="row"> <a href="edit_teacher.php?id=<?= $value['teacher_id'] ?>" class="btn btn-sm btn-warning">Edit</a>

                    </td>
                    <td scope="row"><a href="delete.php?id=<?= $value['teacher_id']; ?>" class="btn btn-sm btn-danger" onclick="alert('are you sure?')">Delete</a>
...
<div class="pagination m-auto" style="width: fit-content;">
     ...
     $page_qry = "SELECT * FROM teacher";
     if (!empty($search_query)) {
          $page_qry .= " WHERE teacher LIKE :search_query OR location LIKE :search_query";
     }
