...
$class_query = "SELECT class.class_id, class.class_name, teacher.teacher_id, teacher.firstname, teacher.lastname FROM class
                LEFT JOIN teacher ON class.teacher_id = teacher.teacher_id";
if (!empty($search_query)) {
     $class_query .= " WHERE class_name LIKE :search_query OR firstname LIKE :search_query";
}

$class_query .= " LIMIT :start_page, :record_per_page";

$s = $pdo->prepare($class_query);
...
<div class="header w-auto shadow m-auto p-3">
     <nav class="navbar navbar-expand-lg bg-body-tertiary">
          <div class="container-fluid">
               <a class="navbar-brand" href="class_index.php">class List</a>
               ...
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                         <li class="nav-item">
                              <a class="nav-link active" aria-current="page" href="class_index.php">Home</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link" href="add_class.php">Add New +</a>
                         </li>
          ...
          <tbody>
               ...
               <?php foreach ($results as $key => $value) { 
                ?>
                    ...
                    <th><?= ++$key ?></th>
                    <!-- <td scope="row"><?= $value['class_id'] ?></td> -->
                    <td scope="row"><?= $value['class_name'] ?></td>

              
                    <td scope="row"><?= $value['firstname'] . ' ' . $value['lastname'] ?></td>

                    <td scope="row"><a href="class_detail.php?id=<?= $value['class_id'] ?>" class="btn btn-sm btn-info">Detail</a>
                    </td>
                    <td scope="row"> <a href="edit_class.php?id=<?= $value['class_id'] ?>" class="btn btn-sm btn-warning">Edit</a>

                    </td>
                    <td scope="row"><a href="delete.php?id=<?= $value['class_id']; ?>" class="btn btn-sm btn-danger" onclick="alert('are you sure?')">Delete</a>
...
<div class="pagination m-auto" style="width: fit-content;">
     ...
     $page_qry = "SELECT * FROM class";
     if (!empty($search_query)) {
          $page_qry .= " WHERE class LIKE :search_query OR location LIKE :search_query";
     }
